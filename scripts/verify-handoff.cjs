...
function isJourneyIndexBuildCommand(script) {
  return typeof script === 'string' && /^npm run build:journey-index(?:\s|$)/.test(script);
}
...
function main() {
  ...
  const required = [
    ...
    'scripts/mtr-stations.cjs',
    'scripts/verify-journey-index.cjs',
    'src/data/kmb.json',
    'src/data/mtr_stations.json',
  ...
  if (!isJourneyIndexBuildCommand(packageJson.scripts?.web)) {
  ...
  for (const item of required.filter((value) => value.startsWith('public/data/journey/'))) {
    const target = path.join(root, item);
    if (fs.existsSync(target) && fs.statSync(target).size <= 2) missing.push(`non-empty ${item}`);
  }
  ...
  try {
    ...
    if (!Array.isArray(kmb.stops) || kmb.stops.length < 1000) missing.push('complete KMB stop snapshot in src/data/kmb.json');
...
module.exports = { isJourneyIndexBuildCommand };
