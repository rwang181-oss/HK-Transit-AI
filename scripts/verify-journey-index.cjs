...
const PROVIDER_IDS = ['KMB', 'CTB', 'GMB', 'MTR'];
...
function assertProviderCoverage(routes) {
  const providerCounts = Object.fromEntries(PROVIDER_IDS.map((provider) => [provider, 0]));
  for (const route of Object.values(routes || {})) {
    if (route && route.provider in providerCounts) providerCounts[route.provider] += 1;
  }
  for (const [provider, count] of Object.entries(providerCounts)) {
    if (count === 0) throw new Error(`${provider} routes are missing from journey index`);
  }
...
function main() {
  ...
  let providerCounts;
  try {
    providerCounts = assertProviderCoverage(routes);
  } catch (error) {
    fail(error.message);
  }
  ...
  console.log(`journey-index verification: PASS (${hubs.length} hubs, ${Object.keys(routes).length} routes, ${Object.keys(cells).length} cells, ${neighborBytes} transfer-index bytes, KMB ${providerCounts.KMB}, CTB ${providerCounts.CTB}, GMB ${providerCounts.GMB}, MTR ${providerCounts.MTR})`);
}

if (require.main === module) main();

module.exports = { assertProviderCoverage };
